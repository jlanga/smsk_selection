...
def process_pvalues(tsv_ete3, tsv_fastcodeml, tsv_output, threshold_pvalue=0.05):
    ...
    # Read tables
    fastcodeml = pd.read_csv(filepath_or_buffer=tsv_fastcodeml, sep="\t")[
        ["orthogroup", "omega_zero", "pvalue"]
    ].assign(program="fastcodeml")

    ete3 = pd.read_csv(filepath_or_buffer=tsv_ete3, sep="\t")[
        ["orthogroup", "omega_zero", "pvalue"]
    ]
    ...
    ete3 = (
        ete3.loc[ete3.orthogroup.isin(fastcodeml_orthogroups)]
        .assign(program="ete3")
        .reset_index(drop=True)
    )

    pvalues = (
        pd.concat([ete3, fastcodeml])
    ...
    bh = (
        pvalues.groupby("orthogroup")
        .apply(
            lambda x: multipletests(x.pvalue, alpha=threshold_pvalue, method="fdr_bh")[
                1
            ]  # Just take the p-values
        )
        .explode()
        .rename("bh")
    )
    ...
    selected = (
        pvalues.groupby("orthogroup")
        .apply(lambda x: all(x.bh < threshold_pvalue))
        .rename("selected")
        .to_frame()
        .reset_index()
    )
...
def parse_arguments():
    ...
    parser.add_argument(
        "-e", "--ete3", help="Path to ete3 p-values (TSV)", required=True
    )
    parser.add_argument(
        "-f", "--fastcodeml", help="Path to fastcodeml p-values (TSV)", required=True
    )
    parser.add_argument("-o", "--output", help="Path to output (TSV)", required=True)
    parser.add_argument(
        "-p",
        "--pvalue",
