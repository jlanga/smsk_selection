...
rule db_parse_pfam:
    input: DOWNLOAD + "Pfam-A.hmm.gz"
    output: DB + "Pfam-A.hmm.gz.pfam_sqlite_bulk_load"
    log: DB + "parse_pfam.log"
    benchmark: DB + "parse_pfam.bmk"
    conda: "db.yml"
    params: DOWNLOAD + "Pfam-A.hmm.gz.pfam_sqlite_bulk_load"
    shell:
        "PFAM_dat_parser.pl {input} 2> {log}; "
        "mv {params} {output} 2>> {log}"
